...
class Webservice_backup extends CI_Controller 

{
...

		{
...

			{
				...
				$res_current_password = $this->db->query("select * from admin where password='$current_password' and user_id='$user_id'");
		...
		public function select_business_categorywise()
		{
			if(isset($_REQUEST['device_id']) && isset($_REQUEST['user_id']) && isset($_REQUEST['city_id']) && isset($_REQUEST['latitude']) && isset($_REQUEST['longitude']) && isset($_REQUEST['category_id_array']))
			{
					...
					{
						// echo "SELECT * , ( 3959 * ACOS( COS( RADIANS(  '$latitude' ) ) * COS( RADIANS( latitude ) ) * COS( RADIANS( longitude ) - RADIANS(  '$longitude' ) ) + SIN( RADIANS(  '$latitude' ) ) * SIN( RADIANS( latitude ) ) ) ) AS distance, information_business.name AS business_name FROM information_business LEFT JOIN category ON information_business.category_id = category.id LEFT JOIN offers ON offers.category_id = category.id LEFT JOIN timing_business ON timing_business.business_id = information_business.id WHERE category.id IN ('$category_id_array') HAVING distance < 10 ORDER BY distance";
		...
		public function select_business_categorywise_old()
		{
			...
			if(isset($_REQUEST['device_id']) && isset($_REQUEST['user_id']) && isset($_REQUEST['city_id']) && isset($_REQUEST['latitude']) && isset($_REQUEST['longitude']) && isset($_REQUEST['category_id_array']))
			{
					...
					$longitude = $_REQUEST['longitude'];
					$category_id_array = $_REQUEST['category_id_array'];
					$category_id_array = explode(',',$category_id_array);
					//$resultsDoExist = 'false';
					...
					elseif($latitude != '' && $longitude != '' && $city_id == '')
					{
						...
						$counter = 0;
						//$row_whole_criteria_total = "";
						$row_whole_criteria_total = array();
		...
		public function select_offers_businesswise()
		{
			if(isset($_REQUEST['device_id']) && isset($_REQUEST['business_id']) && isset($_REQUEST['category_id_array']))
			{
					...
					$res_offers_business = $this->db->query("SELECT *,ib.name as business_name  
														...
														INNER JOIN category AS c ON ib.category_id = c.id
														INNER JOIN menu AS m ON m.business_id = ib.id
		...
		public function filter_discount()
		{
			if(isset($_REQUEST['device_id']) && isset($_REQUEST['user_id']) && isset($_REQUEST['city_id']) && isset($_REQUEST['category_id_array']) && isset($_REQUEST['discount']))
			{
					...
					if($city_id == '' && $discount != '' && $category_id_array != '')
					{
						...
						$res_whole_criteria = $this->db->query("SELECT *,information_business.name as business_name FROM  `offers` INNER JOIN information_business ON offers.business_id = information_business.id INNER JOIN category ON category.id = offers.category_id WHERE category.id IN ('$category_id_array') and offers.discount >= '$discount'");
						$row_whole_criteria = $res_whole_criteria->result();
		...
		// code for favorites status ends here			
					...
					{
						$res_whole_criteria = $this->db->query("SELECT *,information_business.name as business_name FROM  `offers` INNER JOIN information_business ON offers.business_id = information_business.id INNER JOIN category ON category.id = offers.category_id WHERE category.id IN ('$category_id_array')");
						$row_whole_criteria = $res_whole_criteria->result();
		...
		// code for favorites status ends here			
					...
					elseif($city_id != '' && $discount == '' && $category_id_array != '')
					{
						$res_whole_criteria = $this->db->query("SELECT *,information_business.name as business_name FROM  `offers` INNER JOIN information_business ON offers.business_id = information_business.id INNER JOIN category ON category.id = offers.category_id WHERE category.id IN ('$category_id_array') and information_business.city_id='$city_id'");
						$row_whole_criteria = $res_whole_criteria->result();
						if($res_whole_criteria -> num_rows > 0)
						{
								$response['success'] = "1";
		...
		// code for favorites status ends here			
					...
					elseif($city_id != '' && $discount != '' && $category_id_array != '')
					{
						$res_whole_criteria = $this->db->query("SELECT *,information_business.name as business_name FROM  `offers` INNER JOIN information_business ON offers.business_id = information_business.id INNER JOIN category ON category.id = offers.category_id WHERE category.id IN ('$category_id_array') and information_business.city_id = '$city_id' and offers.discount >= '$discount'");
						$row_whole_criteria = $res_whole_criteria->result();
						if($res_whole_criteria -> num_rows > 0)
						{
								$response['success'] = "1";
...
FROM  `offers` INNER JOIN information_business ON offers.business_id = information_business.id INNER JOIN category ON category.id = offers.category_id LEFT JOIN favorite ON favorite.business_id = information_business.id WHERE category.id IN ('$category_id_array') AND favorite.user_id = '$user_id'");
		...
		{
				if(isset($_REQUEST['category_id_array']))
				{
					$category_id_array = $_REQUEST['category_id_array'];
					$category_id_array = explode(',',$category_id_array);
					echo "<pre>";
					...
					foreach ($category_id_array as $single_cat_value) {
							...
							$res_login = $this->db->query("SELECT * FROM  `category` where id='$single_cat_value'");
							$row_login = $res_login->result();
							foreach($row_login as $row)
							{
									$data = array();
		...
		{
			$res_open_close = $this->db->query("SELECT * FROM `timing_business` where business_id = '1'");
			$row_open_close = $res_open_close->result();
			//echo $row_open_close['0']->timing_json_array;
