	                ...
	                <div class="col-sm-12">
						<section class="panel panel-default">
		                      <header class="panel-heading">
		                        <span class="h4">List Client</span>
		                      </header>
							    ...
							    <table class="display table table-bordered table-striped no_mrgn_btm" id="example">
								      <thead>
								        <tr>
								          ...
								          <th width="10%">Plan</th> 
								          <th width="20%">Generate Request for Invoice</th> 
								          <th width="15%"></th> 
								        </tr>
								     </thead>
								     <?php 	
								     	   if($this->session->userdata('email_admin'))
   											{	
								     				$result_client = $this->db->query("select * from admin where type='0'");
								     		}
								     		if($this->session->userdata('email_zone_manager'))
								     		{
								       ...
								       <tr class='gradeA'> 
											...
											{
								       	    ?>
								       	    <a href="javascript:;" class="btn btn-success" onclick="generate_invoice(<?php echo $row->plan_id; ?>,<?php echo $row->id; ?>)">Generate</a>
								       	    <?php } ?>
								       	    </td>
