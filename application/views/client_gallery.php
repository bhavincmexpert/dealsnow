<?php include('header_client.php');
		...
		$select_image_gallery_user_wise = $this->db->query("select gallery.*,gallery_image.*,information_business.*,information_business.id as businessid FROM gallery INNER JOIN gallery_image ON gallery.admin_id = gallery_image.admin_id INNER JOIN information_business ON information_business.admin_id = gallery.admin_id where gallery.category_id = '$categoryid' and gallery.subcategory_id = '$subcatid' and gallery.sub_sub_categoryid = '$subsubcatid' and gallery.admin_id = '$adminid' and information_business.id = '$businessid' GROUP BY gallery_image.image_id");
...
<script>
    $(function () {
        $(".delete").click(function () {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo site_url('client_area/image_remove'); ?>",
                data: {del_id: del_id},
...
<section class="vbox">
    <section class="scrollable padder wrapper">
        <div class="row">
            <form action="<?php echo base_url() . 'index.php/client_information/insert_gallery/'; ?>"
                  data-validate="parsley" enctype="multipart/form-data" method="post">

                <div class="col-sm-12">
                    <header class="panel-heading form_panel">
                        <span class="h4">Edit Your Business</span>
                    </header>
                    <div class="col-md-12">
                        <?php
                        $i = 1;
